 ...
 * 分析意图的完整性，生成澄清问题
 */
export async function analyzeClarification(intent) {
 ...
 * 分析创建视频意图
 */
function analyzeCreateIntent(intent) {
    const questions = [];
    const missingFields = [];
    let completeness = 30;
    // 1. 检查时长
 ...
 * 分析编辑视频意图
 */
function analyzeEditIntent(intent) {
    const questions = [];
    const missingFields = [];
    let completeness = 40;
    const entities = intent.entities;
 ...
 * 分析添加特效意图
 */
function analyzeEffectIntent(intent) {
    const questions = [];
    const missingFields = [];
    let completeness = 40;
    // 检查特效类型
 ...
 * 格式化澄清消息
 */
export function formatClarificationMessage(result) {
