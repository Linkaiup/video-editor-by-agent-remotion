...
export class ValidateStep {
     ...
     */
    async execute(buildArtifact, outputPath) {
     ...
     * 2. Validate 检查（运行时）
     */
    async runValidate(buildArtifact) {
        const errors = [];
        try {
            // 简化版本：检查组件是否可以导入
     ...
     * 3. Snapshot 生成（视觉）
     */
    async generateSnapshots(buildArtifact, outputPath) {
        const frames = [];
        try {
            // 为每个 beat 生成关键帧快照
            for (const comp of buildArtifact.compositions) {
     ...
     * 计算质量评分
     */
    calculateQuality(lint, validate, snapshot) {
        // 流畅度：基于快照数量
