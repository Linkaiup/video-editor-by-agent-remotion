 ...
 * // 返回包含 5-6 个步骤的计划
 */
export async function createTaskPlan(intent) {
    const trace = tracer.startTrace('create_plan', { intent });
    try {
        // 调用 OpenAI API 生成任务计划
 ...
 * });
 */
export async function executeTaskPlan(plan, onStepUpdate) {
 ...
 * 检查素材文件是否存在和可访问
 */
async function validateAssets(step) {
 ...
 * 创建 Remotion 组合规格
 */
async function createComposition(step) {
 ...
 * 生成 React 组件代码
 */
async function generateCode(step) {
 ...
 * 将特效应用到组合中
 */
async function applyEffects(step) {
 ...
 * 渲染视频预览
 */
async function renderPreview(step) {
 ...
 * @returns 是否应该重试
 */
async function shouldRetryStep(step, error) {
    // 可重试的错误类型
 ...
 */
export function getStepProgress(plan) {
