...
export class LLMCodeGenerator {
  ...
  async generateVideoSpec(
    ...
    try {
      ...
      const llmResponse: LLMCodeGenResponse = JSON.parse(jsonMatch[0]);
...
现在开始生成 VideoSpec。`;
   ...
   * 构建用户提示词
   */
  private buildPrompt(beat: Beat, timing: any, design: any, fps: number): string {
...
现在生成 VideoSpec：`;
   ...
   * 验证 VideoSpec 的有效性
   */
  validateVideoSpec(spec: VideoSpec): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
   ...
   * 判断 VideoSpec 的复杂度
   */
  assessComplexity(spec: VideoSpec): 'simple' | 'medium' | 'complex' {
    let score = 0;
