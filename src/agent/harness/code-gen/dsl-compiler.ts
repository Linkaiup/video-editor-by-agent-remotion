...
import type { VideoSpec, LayerConfig, AnimationConfig } from './dsl-types.js';
...
export class DSLCompiler {
   ...
   */
  compile(spec: VideoSpec, componentName: string): string {
...
${componentCode}`;
   ...
   * 生成导入语句
   */
  private generateImports(spec: VideoSpec): string {
    const remotionImports = new Set<string>(['React', 'AbsoluteFill', 'useCurrentFrame']);
...
import { ${remotionList.join(', ')} } from 'remotion';`;
   ...
   * 生成组件代码
   */
  private generateComponent(spec: VideoSpec, componentName: string): string {
...
${layersJSX}
   ...
   * 生成图层代码
   */
  private generateLayerCode(layer: LayerConfig): string {
    // 1. 生成动画变量
    const animationVars = layer.animations.map((anim, idx) =>
      this.generateAnimationVariable(anim, `anim${idx + 1}`)
    ).join('\n');
...
${content}
   ...
   * 生成动画变量
   */
  private generateAnimationVariable(animation: AnimationConfig, varName: string): string {
   ...
   * 生成图层样式
   */
  private generateLayerStyles(layer: LayerConfig): Record<string, any> {
   ...
   * 生成图层内容（JSX）
   */
  private generateLayerContent(layer: LayerConfig): string {
