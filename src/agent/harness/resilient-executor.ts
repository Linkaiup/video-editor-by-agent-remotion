...
import { Harness2Executor, type Harness2Result } from './executor.js';
import { ArtifactsManager } from './artifacts/manager.js';
import type { RebuildOptions, ProjectArtifacts } from './artifacts/types.js';
...
export class ResilientHarness2Executor extends Harness2Executor {
   ...
   * 从 Build 步骤重建
   */
  private async rebuildFromBuild(
    ...
    const buildArtifact = await buildStep.execute(
    ...
    const validationArtifact = await validateStep.execute(
      buildArtifact,
      `${artifacts.projectPath}/artifacts`
    );
   ...
   * 从 Validate 步骤重建
   */
  private async rebuildFromValidate(
    artifacts: ProjectArtifacts,
    options: RebuildOptions
  ): Promise<Harness2Result> {
   ...
   * 从 Storyboard 步骤重建
   */
  private async rebuildFromStoryboard(
    artifacts: ProjectArtifacts,
    options: RebuildOptions
  ): Promise<Harness2Result> {
   ...
   * 应用重试策略
   */
  private applyStrategy(intent: any, strategy: string, attempt: number): any {
