...
export class ArtifactsManager {
    ...
    // ============= Capture Artifact =============
    async saveCapture(artifact) {
        try {
            const capturePath = join(this.artifactsPath, 'capture');
            // 保存 metadata.json
            await writeFile(join(capturePath, 'metadata.json'), JSON.stringify(artifact.metadata, null, 2), 'utf-8');
            // 保存 tokens.json
            await writeFile(join(capturePath, 'tokens.json'), JSON.stringify(artifact.tokens, null, 2), 'utf-8');
            // 更新项目元数据
    ...
    async loadCapture() {
        try {
            const capturePath = join(this.artifactsPath, 'capture');
            const metadata = JSON.parse(await readFile(join(capturePath, 'metadata.json'), 'utf-8'));
            const tokens = JSON.parse(await readFile(join(capturePath, 'tokens.json'), 'utf-8'));
            const artifact = {
                path: capturePath,
                metadata,
                tokens,
    ...
    // ============= Timeline Artifact =============
    async saveTimeline(artifact) {
        try {
            const timelinePath = join(this.artifactsPath, 'timeline.json');
            await writeFile(timelinePath, JSON.stringify(artifact.content, null, 2), 'utf-8');
            // 更新项目元数据
            const metadata = await this.loadMetadata();
            if (metadata) {
    ...
    // ============= Validation Artifact =============
    async saveValidation(artifact) {
        try {
            const validationPath = join(this.artifactsPath, 'validation-report.json');
            await writeFile(validationPath, JSON.stringify(artifact.content, null, 2), 'utf-8');
            // 更新项目元数据
            const metadata = await this.loadMetadata();
            if (metadata) {
...
## 2. Color Palette
${content.colorPalette?.map((c) => `- ${c.name}: ${c.hex} (${c.usage})`).join('\n') || '- Primary: #000000'}
...
## 5. Component Styles
${content.components?.map((c) => `### ${c.name}\n${c.cssSpecs}`).join('\n\n') || 'TBD'}
`;
    }
    parseDesign(content) {
        // 简化版本：返回默认结构
