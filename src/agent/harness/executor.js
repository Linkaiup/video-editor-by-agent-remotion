...
export class Harness2Executor {
    ...
    async execute(intent, assets) {
        ...
        try {
            ...
            const captureArtifact = await this.steps.capture.execute(assets, artifacts.projectPath);
            ...
            const designArtifact = await this.steps.design.execute(captureArtifact, undefined, artifacts.projectPath);
            ...
            const strategyArtifact = await this.steps.strategy.execute(intent, artifacts.projectPath);
            ...
            const storyboardArtifact = await this.steps.storyboard.execute(strategyArtifact, captureArtifact, artifacts.projectPath);
            ...
            const timelineArtifact = await this.steps.timeline.execute(storyboardArtifact, strategyArtifact, artifacts.projectPath);
            ...
            const buildArtifact = await this.steps.build.execute(storyboardArtifact, designArtifact, timelineArtifact, artifacts.projectPath);
            ...
            const validationArtifact = await this.steps.validate.execute(buildArtifact, join(artifacts.projectPath, 'artifacts'));
            ...
            const renderArtifact = await this.steps.render.execute(buildArtifact, timelineArtifact, artifacts.projectPath);
     ...
     * 构建成功消息
     */
    buildSuccessMessage(artifacts, renderArtifact) {
        let message = '✅ 视频生成成功！\n\n';
