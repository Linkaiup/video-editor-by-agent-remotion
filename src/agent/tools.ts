 ...
 * @returns 素材信息对象
 */
export async function validateAsset(assetPath: string): Promise<AssetInfo> {
 ...
 * @returns 保存的文件路径
 */
export async function saveComponentCode(
  code: string,
  componentName: string,
  outputDir: string = './src/compositions'
): Promise<string> {
 ...
 */
async function updateRootFile(compositionId: string, componentPath: string): Promise<void> {
 ...
 * @returns 代码内容
 */
export async function loadComponentCode(filepath: string): Promise<string> {
 ...
 * @returns 帧数
 */
export function parseDuration(duration: string, fps: number): number {
  const patterns = [
    { regex: /(\d+)分\s*(\d+)?秒?/, handler: (m: RegExpMatchArray) => (parseInt(m[1]) * 60 + (parseInt(m[2]) || 0)) * fps },
    { regex: /(\d+)m\s*(\d+)?s?/, handler: (m: RegExpMatchArray) => (parseInt(m[1]) * 60 + (parseInt(m[2]) || 0)) * fps },
    { regex: /(\d+(?:\.\d+)?)秒/, handler: (m: RegExpMatchArray) => parseFloat(m[1]) * fps },
    { regex: /(\d+(?:\.\d+)?)s/, handler: (m: RegExpMatchArray) => parseFloat(m[1]) * fps },
    { regex: /(\d+)毫秒/, handler: (m: RegExpMatchArray) => (parseInt(m[1]) / 1000) * fps },
    { regex: /(\d+)ms/, handler: (m: RegExpMatchArray) => (parseInt(m[1]) / 1000) * fps },
    { regex: /(\d+)帧/, handler: (m: RegExpMatchArray) => parseInt(m[1]) },
    { regex: /(\d+)f/, handler: (m: RegExpMatchArray) => parseInt(m[1]) },
  ];
 ...
 */
export async function processLocalImage(imagePath: string): Promise<string> {
 ...
 * @returns 视频尺寸对象
 */
export function getVideoDimensions(preset: string): { width: number; height: number; fps: number } {
 ...
 * @returns 验证结果
 */
export function validateCompositionSpec(spec: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
