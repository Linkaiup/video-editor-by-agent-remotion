...
import { UserIntent, IntentConfirmation } from './types.js';
 ...
 */
export async function recognizeIntent(userMessage: string): Promise<UserIntent> {
...
export async function confirmIntent(
  ...
  try {
    ...
    if (intent.confidence < 0.7) {
      ...
      clarifications.push(
        `置信度较低 (${intent.confidence.toFixed(2)})。您是想 ${intent.type} 吗？`
      );
    }
 ...
 */
export function extractAssets(message: string): string[] {
  // 定义多种素材匹配模式
  const patterns = [
    // 文件路径（相对或绝对）
 ...
 * // 返回: ["fade", "zoom"]
 */
export function extractEffects(message: string): string[] {
  // 特效关键词映射表
  // 键：特效类型，值：可能的关键词列表
