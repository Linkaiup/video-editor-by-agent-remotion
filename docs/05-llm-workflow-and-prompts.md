...
└────────────────────────────┬────────────────────────────────────┘
                             ↓
┌─────────────────────────────────────────────────────────────────┐
│  📝 LLM 2: 任务规划 (Task Planning)                             │
│  ────────────────────────────────────────────────────────────  │
│  文件：src/agent/task-planning.ts                               │
│                                                                  │
│  System Prompt:                                                 │
│  "你是 Remotion 任务规划师，将意图分解为可执行步骤"               │
│                                                                  │
│  User Prompt:                                                   │
│  为这个意图创建任务计划：                                         │
│  { type: "create", entities: {...} }                            │
│  返回 JSON: { steps: [{action, description}] }                  │
│                                                                  │
│  输出：                                                          │
│  {                                                              │
│    "steps": [                                                   │
...
| 特性 | Template 模式 | LLM DSL 模式 | 直接生成代码（未采用） |
|------|--------------|--------------|---------------------|
